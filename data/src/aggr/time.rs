...
impl TimeSeries<KlineDataPoint> {
    ...
    pub fn insert_trades_or_create_bucket(&mut self, buffer: &[Trade]) {
        ...
        let mut updated_times = rustc_hash::FxHashSet::default();
    ...
    pub fn insert_trades_existing_buckets(&mut self, buffer: &[Trade]) {
        self.insert_trades_existing_buckets_inner(buffer, true);
    }

    /// Like `insert_trades_existing_buckets` but skips per-batch POC
    /// recalculation. Use this during bulk backfill where POC will be
    /// recalculated once at the end via `update_poc_status`.
    pub fn insert_trades_existing_buckets_no_poc(&mut self, buffer: &[Trade]) {
        self.insert_trades_existing_buckets_inner(buffer, false);
    }

    fn insert_trades_existing_buckets_inner(&mut self, buffer: &[Trade], recalc_poc: bool) {
        if buffer.is_empty() {
            return;
        }
        let aggr_time = self.interval.to_milliseconds();
    ...
    pub fn change_tick_size(&mut self, tick_size: PriceStep) {
        ...
        // Raw trades are no longer retained after aggregation.
        // Footprint data is cleared; the fetch cycle will re-populate
        // buckets from exchange/cache on the next render pass.
        self.clear_trades();
    }

    pub fn update_poc_status(&mut self) {
        self.update_poc_status_windowed(self.datapoints.len());
    }

    /// Like `update_poc_status` but only processes the most recent `lookback`
    /// bars. The NPoC study renders at most `lookback` bars, so there is no
    /// benefit computing status for bars further back. This reduces the
    /// worst-case complexity from O(n²) to O(lookback²), which matters when
    /// thousands of bars are loaded after a large historical backfill.
    pub fn update_poc_status_windowed(&mut self, lookback: usize) {
        // Collect only the tail of bars we care about.
        let updates: Vec<(u64, Price)> = self
            .datapoints
            .iter()
            .rev()
            .take(lookback)
    ...
    pub fn suggest_trade_fetch_range(
        ...
        // Only look for gaps within the currently visible window.
        // Scanning all loaded klines would find empty footprints for bars
        // that are loaded but far outside the view, causing the fetch cycle
        // to re-trigger indefinitely even after the visible range is fully
        // backfilled.
        let has_gap_in_view = self
            .datapoints
            .range(visible_earliest..=visible_latest)
            .any(|(_, dp)| dp.footprint.trades.is_empty());

        if !has_gap_in_view {
            return None;
        }

        // There is at least one empty bucket inside the visible window.
        // Find the bracketing trade times so we know the exact unfilled span.
        let last_t_before_gap = self
            .datapoints
            .range(..visible_earliest)
            .rev()
            .find_map(|(_, dp)| dp.last_trade_time());

        let first_t_after_gap = self
            .datapoints
            .range(visible_latest + 1..)
            .find_map(|(_, dp)| dp.first_trade_time());

        let fetch_from = last_t_before_gap
            .map_or(visible_earliest, |t| t.saturating_add(1))
            .max(visible_earliest);
        let fetch_to = first_t_after_gap
            .map_or(visible_latest, |t| t.saturating_sub(1))
            .min(visible_latest);
