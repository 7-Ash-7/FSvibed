...
impl TickAggr {
    ...
    pub fn change_tick_size(&mut self, tick_size: PriceStep) {
        ...
        // Datapoints are cleared; a new fetch will rebuild from source.
    ...
    pub fn update_poc_status(&mut self) {
        let lookback = 512.min(self.datapoints.len());
        let total_points = self.datapoints.len();
        // Only process the most recent `lookback` bars — the NPoC study renders
        // at most that many, and the naive full scan is O(n²).
        let start_idx = total_points.saturating_sub(lookback);

        let updates: Vec<(usize, Price)> = self
            .datapoints
            .iter()
            .enumerate()
            .skip(start_idx)
