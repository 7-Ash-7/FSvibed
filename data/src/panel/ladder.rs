...
const TRADE_RETENTION_MS: u64 = u64::MAX;
...
impl std::fmt::Display for LookbackUnit {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            LookbackUnit::Hours => write!(f, "Hours"),
            LookbackUnit::Days => write!(f, "Days"),
        }
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Deserialize, Serialize)]
pub struct BackfillLookback {
    pub amount: u32,
    pub unit: LookbackUnit,
}

impl BackfillLookback {
    pub fn to_millis(self) -> u64 {
        let ms_per_unit = match self.unit {
            LookbackUnit::Hours => 3_600_000_u64,
            LookbackUnit::Days => 86_400_000_u64,
        };
        self.amount as u64 * ms_per_unit
    }
}

impl Default for BackfillLookback {
    fn default() -> Self {
        Self {
            amount: 1,
            unit: LookbackUnit::Hours,
        }
    }
}
...
pub enum ViewMode {
    #[default]
    Original,
    /// Right panel shows ask bar (base) + bid bar stacked on top, both from right edge.
    StackedVP,
    /// Right panel shows only a delta bar (bid - ask), coloured by dominant side.
    DeltaVP,
}
...
impl std::fmt::Display for ViewMode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ViewMode::Original => write!(f, "Original"),
            ViewMode::StackedVP => write!(f, "Stacked VP"),
            ViewMode::DeltaVP => write!(f, "Delta VP"),
        }
    }
}

impl ViewMode {
    pub const ALL: &'static [ViewMode] = &[ViewMode::Original, ViewMode::StackedVP, ViewMode::DeltaVP];
}
...
pub struct Config {
    ...
    pub show_trade_text: bool,
    #[serde(deserialize_with = "ok_or_default", default)]
    pub cumulative_orders: bool,
    #[serde(deserialize_with = "ok_or_default", default)]
    pub cumulative_trades: bool,
    #[serde(deserialize_with = "ok_or_default", default)]
    pub backfill_lookback: BackfillLookback,
    #[serde(deserialize_with = "ok_or_default", default)]
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            show_trade_text: true,
            cumulative_orders: false,
            cumulative_trades: false,
            backfill_lookback: BackfillLookback::default(),
