...
/// Thin dashed line matching the last-price marker style (2px segments).
/// Used for the crosshair so it is visually lighter than the thicker 4px variant.
pub fn thin_dashed_line(theme: &'_ Theme) -> Stroke<'_> {
    let palette = theme.extended_palette();
    Stroke::with_color(
        Stroke {
            width: 1.0,
        ...
        palette
            ...
            .strong
            .color
            .scale_alpha(if palette.is_dark { 0.6 } else { 0.8 }),
    )
}
