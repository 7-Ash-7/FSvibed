...
impl Dashboard {
    ...
    pub fn update(
        ...
        match message {
            ...
            Message::Pane(window, message) => match message {
                ...
                pane::Message::SetGlobalCrosshair(value) => {
                    self.iter_all_panes_mut(main_window.id)
                        .for_each(|(_, _, state)| {
                            state.content.set_crosshair(value);
                            state.settings.visual_config =
                                state.settings.visual_config.clone().map(|cfg| {
                                    match cfg {
                                        data::layout::pane::VisualConfig::Kline(mut k) => {
                                            k.show_crosshair = value;
                                            data::layout::pane::VisualConfig::Kline(k)
                                        }
                                        other => other,
                                    }
                                });
            ...
            Message::ChangePaneStatus(pane_id, status) => {
                if let Some(pane_state) = self.get_mut_pane_state_by_uuid(main_window.id, pane_id) {
                    ...
                    // When a fetch task reports completion, ensure the trade-fetch
                    // flag on the kline chart is always cleared — even if the last
                    // data batch never reached `until_time` (e.g. historical data
                    // ending before the requested range). Without this the chart
                    // would permanently believe a fetch is in progress and refuse
                    // to request any further trade ranges.
                    if matches!(status, pane::Status::Ready) {
                        if let pane::Content::Kline { chart, .. } = &mut pane_state.content
                            && let Some(c) = chart
                        {
                            c.on_fetch_task_complete();
                        }
                    }
                }
            }
    ...
    pub fn distribute_fetched_data(
        ...
        match data {
            FetchedData::Trades { batch, until_time, req_id } => {
                ...
                if last_trade_time < until_time {
                    ...
                    if let Err(reason) =
                        self.insert_fetched_trades(main_window, pane_id, &filtered_batch, true, req_id)
                    {
                        return self.handle_error(Some(pane_id), &reason, main_window);
                    }
                }
    ...
    fn insert_fetched_trades(
        ...
        match &mut pane_state.content {
            pane::Content::Kline { chart, .. } => {
                if let Some(c) = chart {
                    c.insert_raw_trades(trades, is_batches_done, req_id);
            ...
            pane::Content::Ladder(panel) => {
                if let Some(p) = panel {
                    p.insert_trades(trades);
                    pane_state.status = pane::Status::Ready;
                    Ok(())
                } else {
                    Err(DashboardError::Unknown(
                        "fetched trades but no ladder found".to_string(),
                    ))
                }
            }
        ...
        self.iter_all_panes_mut(main_window)
            .for_each(|(_window_id, _pane, state)| match state.tick(now) {
                ...
                Some(pane::Action::Panel(action)) => {
                    match action {
                        panel::Action::RequestBackfill { ticker_info: _, from_ms } => {
                            let pane_id = state.unique_id();
                            let ready_streams = state
                                .streams
                                .ready_iter()
                            ...
                            let trade_stream = ready_streams.iter().find_map(|s| {
                                if let StreamKind::Trades { ticker_info: ti } = s {
                                    Some((*ti, *s))
                                } else {
                                    None
                                }
                            ...
                            if let Some((ti, stream)) = trade_stream {
                                ...
                                let to_ms = std::time::SystemTime::now()
                                    .duration_since(std::time::UNIX_EPOCH)
                                    .unwrap_or_default()
                                    .as_millis() as u64;

                                let task = if is_hyperliquid {
                                    let req_id = uuid::Uuid::new_v4();
                                    fetcher::hl_recent_trades_task(
                                        layout_id,
                                        pane_id,
                                        ti,
                                        stream,
                                    ...
                                    fetcher::binance_ladder_backfill_task(
                                        layout_id,
                                        pane_id,
                                        ti,
                                        stream,
