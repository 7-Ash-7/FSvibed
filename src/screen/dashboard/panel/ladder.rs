    ...
    fn draw(
        ...
        let orderbook_visual = self.cache.draw(renderer, bounds.size(), |frame| {
            if let Some(grid) = self.build_price_grid() {
                ...
                if self.config.show_chase_tracker {
                    // Left gap: between order columns and price
                    let left_gap_mid_x = cols.ask_order.1 + (layout.inside_pad_px + COL_PADDING) * 0.5;
                    // Right gap: between price and trade columns
                ...
                // Divider between order columns and price column
                draw_vsplit(cols.ask_order.1, spread_row);
                // Divider between price column and trade columns
                draw_vsplit(cols.price.1, spread_row);
                // Right axis — slightly brighter than divider
                {
...
struct ColumnRanges {
    ...
    sell: (f32, f32),
    buy: (f32, f32),
    /// x position of the right-side volume axis
    right_axis: f32,
}
...
impl Ladder {
    ...
    fn column_ranges(&self, width: f32, price_px: f32) -> ColumnRanges {
        // New layout: [BidOrder→][AskOrder→]  [Price]  [←SellTrade][←BuyTrade] |axis
        // 5 gaps: bid_order|ask_order, ask_order|price, price|sell, sell|buy, buy|axis
        const N_GAPS: f32 = 5.0;
        let total_gutter_width = COL_PADDING * N_GAPS;
        ...
        // Left side: both order columns side by side
        let bid_order_width = order_share * 0.5;
        let ask_order_width = order_share * 0.5;
        // Right side: both trade columns, with axis at far right
    ...
    fn draw_row(
        ...
        // Slightly shorter bars to leave a 1px gap between rows
        let bar_h = ROW_HEIGHT - 1.0;

        // Only draw the relevant order column for this row
        // Bid rows use bid_order (leftmost column), ask rows use ask_order (next column)
        if is_bid {
            Self::fill_bar(
                frame,
                cols.bid_order,
            ...
            if show_trade_text {
                ...
                Self::draw_cell_text(frame, &qty_txt, cols.bid_order.0 + 4.0, y, text_color, Alignment::Start);
            }
        } else {
            Self::fill_bar(
                frame,
            ...
            if show_trade_text {
                let qty_txt = self.format_quantity(order_qty);
                Self::draw_cell_text(frame, &qty_txt, cols.ask_order.0 + 4.0, y, text_color, Alignment::Start);
            }
        }

        // Trade columns: both grow leftward from right_axis
        // Sell: left-of-buy, grows leftward from its right edge
        Self::fill_bar(
            frame,
            cols.sell,
            y,
        ...
        let sell_txt = if show_trade_text && trade_sell_qty_f32 > 0.0 {
        ...
        // Buy: grows leftward from right_axis
        // Use (cols.sell.0, cols.right_axis) so max bar fills from right_axis leftward
        // but only up to the sell column start
        Self::fill_bar(
            frame,
            (cols.sell.0, cols.right_axis),
            y,
            bar_h,
            trade_buy_qty_f32,
            max_trade_qty,
        ...
        let buy_txt = if show_trade_text && trade_buy_qty_f32 > 0.0 {
    ...
    fn fill_bar(
        ...
        if let Some(outline) = outline_color {
            ...
            // Top, outer tip only — omit bottom (next row's top covers it) and inner edge
    ...
    fn visible_rows(&self, bounds: Rectangle, grid: &PriceGrid) -> (Vec<VisibleRow>, Maxima) {
        ...
        // Rows above spread (asks, idx<0) accumulate downward (top→spread)
        // Rows below spread (bids, idx>0) accumulate upward (bottom→spread)
        if self.config.cumulative_trades {
            // Find split index (spread/divider row)
            let split = visible.iter().position(|r| {
                matches!(r.row, DomRow::Spread | DomRow::CenterDivider)
            });
            if let Some(split_idx) = split {
                // Ask side: rows before split, accumulate downward (top to split)
                ...
                // Bid side: rows after split, accumulate upward (bottom to split)
                let mut cum_buy = Qty::default();
                let mut cum_sell = Qty::default();
                ...
                // Recalc maxima for cumulative values
        ...
        // Cumulative orders: accumulate order book qty from extremes toward spread
