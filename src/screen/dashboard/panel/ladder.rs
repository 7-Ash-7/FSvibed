    ...
    fn draw(
        ...
        let orderbook_visual = self.cache.draw(renderer, bounds.size(), |frame| {
            if let Some(grid) = self.build_price_grid() {
                ...
                if self.config.show_chase_tracker {
                    // Left gap: between order column and price
                    let left_gap_mid_x = cols.order.1 + (layout.inside_pad_px + COL_PADDING) * 0.5;
                    // Right gap: between price and trade column
                ...
                // Left divider: between order column and price
                draw_vsplit(cols.order.1, spread_row);
                // Right divider: between price and trade column
                draw_vsplit(cols.price.1, spread_row);
                // Right axis line — slightly brighter
                {
...
impl Ladder {
    ...
    fn column_ranges(&self, width: f32, price_px: f32) -> ColumnRanges {
        // Layout: [Order→] [Price] [←Trades] |axis
        // 3 column gaps + 1 final padding = 3 gaps
        const N_GAPS: f32 = 3.0;
        let total_gutter_width = COL_PADDING * N_GAPS;
        let usable_width = (width - total_gutter_width - COL_PADDING).max(0.0);
        ...
        let order_width = if rest_ratio > 0.0 {
            (ORDER_QTY_COLS_WIDTH / rest_ratio) * rest
        } else {
            0.0
        };
        let trade_width = if rest_ratio > 0.0 {
        ...
        let order_end = cursor_x + order_width;
        let order_range = (cursor_x, order_end);
        cursor_x = order_end + COL_PADDING;

        let price_end = cursor_x + price_width;
        let price_range = (cursor_x, price_end);
        cursor_x = price_end + COL_PADDING;

        let trade_end = cursor_x + trade_width;
        let trade_range = (cursor_x, trade_end);
        let right_axis = trade_end + COL_PADDING;
    ...
    fn draw_row(
        ...
        if show_trade_text && order_qty_f32 > 0.0 {
        ...
        // Trade column: sell bars drawn first (behind), then buy bars on top.
        // Both grow leftward from right_axis. The wider bar is always visible underneath.
        // Sell (ask trades) - base layer
        Self::fill_bar(
            frame,
            cols.trade,
            y,
            ...
            max_trade_qty,
            trade_sell_color,
            false,  // grows leftward from trade.1 = right_axis
            trade_alpha,
            outline_color,
        );
        // Buy (bid trades) - drawn on top
        Self::fill_bar(
            frame,
            cols.trade,
            y,
        ...
        // Text: both sit just left of right_axis, sell text further left of buy text
        if show_trade_text {
            ...
            if trade_buy_qty_f32 > 0.0 {
                ...
                // Buy text sits just right of right_axis
                Self::draw_cell_text(
                    frame, &buy_txt,
                    cols.right_axis + 4.0, y,
