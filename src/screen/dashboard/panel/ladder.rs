...
/// Used for streaming diff exchanges (Binance, Bybit etc.) where updates are frequent.
/// For snapshot-based exchanges (Hyperliquid) we skip the gap-reset entirely since
/// updates naturally arrive much less frequently.
...
impl Ladder {
    ...
    pub fn insert_depth(&mut self, depth: &Depth, update_t: u64) {
        ...
        if self.config.show_chase_tracker {
            // For snapshot-based exchanges (e.g. Hyperliquid) depth updates arrive
            // infrequently, so we pass Duration::ZERO to disable the gap-reset that
            // would otherwise fire between every snapshot. Streaming diff exchanges
            // (Binance, Bybit) keep the 200ms guard so stale chases reset promptly.
            let max_int = if self.ticker_info.exchange().is_depth_client_aggr() {
                CHASE_MIN_INTERVAL
            } else {
                Duration::ZERO
            };
...
impl Ladder {
    ...
    fn draw_original(
        ...
        if self.config.show_chase_tracker {
            // Trails sit directly on the vertical separator lines.
            let left_sep_x = cols.order.1.floor() + 0.5;
            let right_sep_x = cols.price.1.floor() + 0.5;
    ...
    fn draw_vp(
        ...
        // For StackedVP: scale against the maximum *combined* (bid + ask) qty across all
        // visible rows, so the widest combined bar exactly fills vp_width and individual
        // bars are always proportionally sized relative to one another.
        //
        // For DeltaVP: scale against the maximum individual qty (bid or ask), which is
        // the natural reference for a single signed bar.
        let max_vp_qty: f32 = if stacked {
            visible_rows.iter().map(|r| {
                f32::from(r.buy_t) + f32::from(r.sell_t)
        ...
        let mut _spread_row: Option<(f32, f32)> = None;
        let mut best_bid_y: Option<f32> = None;
        let mut best_ask_y: Option<f32> = None;
            ...
            match visible_row.row {
                DomRow::Ask { price, qty } | DomRow::Bid { price, qty } => {
                    ...
                    if max_vp_qty > 0.0 {
                        ...
                        if stacked {
                            // StackedVP: both bars are scaled against the maximum combined
                            // (bid + ask) total, so proportions are preserved across all rows
                            // and the combined bar never overflows vp_width.
                            let ask_bar_w = ((ask_qty_f32 / max_vp_qty) * vp_width).min(vp_width);
                            let bid_bar_w = ((bid_qty_f32 / max_vp_qty) * vp_width).min(vp_width);
                            ...
                            let delta_bar_w = ((abs_delta / max_vp_qty) * vp_width).min(vp_width);
                ...
                DomRow::Spread => {
                    if let Some(spread) = self.raw_price_spread {
                        let min_ticksize = self.ticker_info.min_ticksize;
                        _spread_row = Some((y, y + ROW_HEIGHT));
                        let spread = spread.round_to_min_tick(min_ticksize);
                        let content = format!("Spread: {}", spread.to_string(min_ticksize));
                        let price_x_center = (cols.price.0 + cols.price.1) * 0.5;
                        frame.fill_text(Text {
    ...
    fn draw_chase_trail(
        ...
        if let Some((start_p_raw, end_p_raw, _alpha)) = tracker.segment() {
