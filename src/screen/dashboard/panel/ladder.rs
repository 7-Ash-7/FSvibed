...
impl Ladder {
    ...
    pub fn invalidate(&mut self, now: Option<Instant>) -> Option<super::Action> {
        ...
        if !self.backfill_done && self.last_exchange_ts_ms.is_some() {
...
impl Ladder {
    ...
    fn column_ranges(&self, width: f32, price_px: f32) -> ColumnRanges {
        ...
        //   Order:      0% .. ~25%
        //   Price:     ~25% .. ~50%  (centred)
        //   Ask trade: ~50% .. 75%   (grows leftward from right_axis)
        //   Bid trade:  75% .. 100%  (grows rightward from right_axis)
        //
        // right_axis = 75% of width
        let right_axis = (width * 0.75).floor();
        let price_mid   = (width * 0.50).floor();

        // Price column centred around 50%, width clamped to price_px
        ...
        // Order column: from 0 up to price_start - padding
        let order_end   = (price_start - COL_PADDING).max(0.0);
        let order_range = (0.0, order_end);

        // Trade column: from price_end + padding to right_axis
        // Ask bars grow leftward from right_axis inside this column
    ...
    fn draw_row(
        ...
        // Price — use neutral text colour (theme Text setting) rather than
        // side colour, so the DOM is easier to read at a glance.
