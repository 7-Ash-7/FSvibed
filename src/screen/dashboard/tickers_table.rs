...
impl TickersTable {
        ...
        for idx in win.first..win.last {
            match vcfg.virtual_to_item(idx) {
                VirtualItemIndex::Gap => {
                    cards = cards.push(Self::favorites_separator(sep_block_height));
                }
                VirtualItemIndex::Row(data_idx) => {
                    ...
                    if let Some(display_data) = self.display_cache.get(&row_ref.ticker) {
                        cards = cards.push(Self::ticker_card(
                            &row_ref.ticker,
                            display_data,
    ...
    fn separator_height(fav_n: usize) -> f32 {
        if fav_n > 0 {
    ...
    fn favorites_separator<'a>(sep_block_height: f32) -> Element<'a, Message> {
        container(
    ...
    /// Favourited tickers are always pinned to the top, sorted alphabetically by display
    /// symbol. Non-favourited tickers appear below, sorted by the active sort option.
    /// When a search query is active, match-quality bucket is the primary tiebreaker in
    /// both sections before their respective secondary ordering.
    fn filtered_rows<'a>(
        &'a self,
        search_upper: &str,
        ...
        // Favourited tickers: always collected and pinned to the top regardless of any
        // toggle. Sorted alphabetically so the user can find them reliably.
        let mut fav_rows: Vec<_> = self
            .ticker_rows
            .iter()
            .filter(|row| {
                row.is_favorited
        ...
        // Sort: search-rank bucket first (exact matches surface when searching), then
        // alphabetically by display symbol, then raw candidate length as tiebreak.
        fav_rows.sort_by(|(a, ra), (b, rb)| {
            (ra.bucket, ra.pos)
                .cmp(&(rb.bucket, rb.pos))
                .then_with(|| {
                    let sym_a = a.ticker.display_symbol_and_type().0;
                    let sym_b = b.ticker.display_symbol_and_type().0;
                    sym_a.cmp(&sym_b)
                })
        ...
        // Non-favourited rows: appear below favourites, sorted by the active sort option.
    ...
    fn ticker_card<'a>(
        ...
        // Star button on every card — clicking it immediately toggles favourite status.
        let star_btn = button(if is_fav {
        ...
        .width(24)
        .height(Length::Fill)
        .style(move |theme, status| style::button::transparent(theme, status, is_fav));
            ...
            row![
                ...
                button(
                    row![
                        ...
                        .padding(padding::left(8).right(4).bottom(4).top(4))
                        .spacing(4),
                    ]
                    .align_y(Alignment::Center),
                )
                .style(style::button::ticker_card)
                .on_press(Message::TickerSelected(*ticker, None))
                .width(Length::Fill),
                star_btn,
            ]
            .align_y(Alignment::Center),
...
struct VirtualListConfig {
    ...
    /// Used for the favourites separator in the full view. None for compact view.
