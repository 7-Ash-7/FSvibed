...
pub enum BarClass {
    ...
    /// draw a single bar: success color when value >= baseline, danger color when below.
    Signed,
}
...
pub struct BarPlot<V, CL, T> {
    ...
    pub padding: f32,
    pub classify: CL, // Single vs Overlay with signed overlay vs Signed
    ...
    CL: Fn(&S::Y) -> BarClass,
{
    fn y_extents(&self, datapoints: &S, range: RangeInclusive<u64>) -> Option<(f32, f32)> {
        ...
        // Determine the baseline value for extent purposes
        let baseline_v = match self.baseline {
            Baseline::Zero => 0.0,
            Baseline::Min => min_v,
            Baseline::Fixed(v) => v,
        };

        // For Zero baseline, if all values are <= 0, return None only if truly nothing to draw
        if matches!(self.baseline, Baseline::Zero) && max_v <= 0.0 && min_v >= 0.0 {
            return None;
        }

        // Include both the baseline and the data range so negative bars render
        let low = min_v.min(baseline_v);
        let high = max_v.max(baseline_v);
        ...
        if self.padding > 0.0 {
            let span = high - low;
            highest = high + span * self.padding;
        }
        let lowest = low - if self.padding > 0.0 {
            (high - low) * self.padding
        } else {
            0.0
        };
    ...
    fn draw(
        ...
        datapoints.for_each_in(range, |x, y| {
            ...
            let (top_y, h_total) = if rel > 0.0 {
                // Bar extends upward from baseline
                let y_total = scale.to_y(total);
                let h = (y_base - y_total).max(0.0);
                (y_total, h)
            } else {
                // Bar extends downward from baseline
                let y_total = scale.to_y(total);
            ...
            match (self.classify)(y) {
                ...
                BarClass::Signed => {
                    // Green when value >= baseline, red when below
