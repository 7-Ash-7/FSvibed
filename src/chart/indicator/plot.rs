...
use crate::style::{self, thin_dashed_line};
use data::util::{guesstimate_ticks, round_to_tick};

use iced::widget::canvas::{self, Cache, Geometry, LineDash, Path, Stroke};
    ...
    S: Series,
{
    ...
    /// If set, a thin dashed horizontal line is drawn at this value
    /// (matching the main chart's last-price line style).
    ...
    S: Series,
{
    ...
    fn draw(
        ...
        let crosshair = self.crosshair_cache.draw(renderer, bounds.size(), |frame| {
            let dashed = thin_dashed_line(theme);

            // Last-value line: always visible, screen space, mirrors main chart price line.
            if let Some(last_val) = self.last_value {
                let min = self.min_for_labels;
                let max = self.max_for_labels;
                if max > min {
                    let screen_y = bounds.height - ((last_val - min) / (max - min)) * bounds.height;
                    let palette = theme.extended_palette();
                    let line_color = palette.secondary.strong.color
                        .scale_alpha(if palette.is_dark { 0.5 } else { 0.7 });
                    frame.stroke(
                        &Path::line(
            ...
            if let Some(cursor_position) = cursor.position_in(ctx.bounds) {
                ...
                // Cursor is in the indicator pane.
                // Vertical line: reuse the same x-snap logic as the main chart branch.
                let width = frame.width() / ctx.scaling;
                let region = Rectangle {
                    x: -ctx.translation.x - width / 2.0,
                    y: 0.0,
                ...
                let snap_ratio_x = match ctx.basis {
                    Basis::Time(tf) => {
                        let step = tf.to_milliseconds() as f64;
                        let crosshair_ratio = f64::from(cursor_position.x / bounds.width);
                        let rx = ((earliest + crosshair_ratio * (latest - earliest)) / step).round()
                ...
                frame.stroke(
                    &Path::line(
                        Point::new(snap_ratio_x * bounds.width, 0.0),
                        Point::new(snap_ratio_x * bounds.width, bounds.height),
                    ),
                    dashed,
                );

                // Horizontal line: snap to nearest tick value
                ...
                let snap_ratio_y = (rounded - highest) / (lowest - highest);

                frame.stroke(
                    &Path::line(
                        Point::new(0.0, snap_ratio_y * bounds.height),
                        Point::new(bounds.width, snap_ratio_y * bounds.height),
                    ),
                    dashed,
                );
            }
