...
//   Rendered as a histogram centred on 1.0 (green above, red below).
...
impl CvdIndicator {
    ...
    fn alpha(period: u32) -> f32 {
        // period=0 is a transient state while the user has cleared the input box.
        // Clamp to 1 so alpha=1.0 (pass-through, no smoothing) rather than 2.0
        // which would produce diverging oscillations.
        let p = period.max(1);
        2.0 / (p as f32 + 1.0)
    }
    ...
    fn rebuild(&mut self, source: &PlotData<KlineDataPoint>) {
        ...
        match source {
            PlotData::TimeBased(ts) => {
                // Cap at the most recent 5000 bars. CVD is cumulative but
                // anchoring it to this window is visually equivalent for
                // any chart showing a subset of history — the shape is
                // identical, only the absolute level differs, which is
                // meaningless for delta analysis. Without this cap, rebuild
                // is O(n * price_levels) and freezes for large backlills.
                let iter = ts.datapoints.iter().rev().take(5000).collect::<Vec<_>>();
                for (timestamp, dp) in iter.into_iter().rev() {
            ...
            PlotData::TickBased(ta) => {
                ...
                for (idx, dp) in ta.datapoints.iter().enumerate().skip(start) {
                    let value = self.compute_bar(&dp.footprint, alpha);
    ...
    fn indicator_elem<'a>(
        ...
        if self.config.ema_mode {
            self.ema_histogram_elem(main_chart, visible_range, axis_left)
        } else {
            self.classic_line_elem(main_chart, visible_range, axis_left)
        }
    }

    fn classic_line_elem<'a>(
        &'a self,
        main_chart: &'a ViewState,
        visible_range: RangeInclusive<u64>,
        axis_left: bool,
    ...
    fn ema_histogram_elem<'a>(
        ...
        let tooltip = |ratio: &f32, _next: Option<&f32>| {
            PlotTooltip::new(format!("EMA Ratio: {:.4}", ratio))
        };

        // The value function returns the raw ratio; baseline is Fixed(1.0) so
        // bars extend above/below 1.0. BarClass::Signed colours green >= 1 / red < 1.
        let plot = BarPlot::new(
            |v: &f32| *v,
            |_: &f32| BarClass::Signed,
        )
        ...
        // Pass 1.0 as last_value so the existing dashed-line mechanism in
        // ChartCanvas draws a subtle horizontal reference at the neutral level.
        indicator_row(
            main_chart,
            &self.cache,
            plot,
...
impl KlineIndicatorImpl for CvdIndicator {
    ...
    fn element<'a>(
        ...
        self.indicator_elem(chart, visible_range, axis_left)
    }
