...
///
/// `last_value` — if `Some`, draws a persistent dashed horizontal line at that
/// value and a highlighted label on the y-axis, mirroring the main chart's
/// last-price line behaviour.
...
impl canvas::Program<Message> for IndicatorLabel<'_> {
    ...
    fn draw(
        ...
        let labels = self.label_cache.draw(renderer, bounds.size(), |frame| {
            ...
            // Last-value label — highlighted box, always visible
            if let Some(last_val) = self.last_value {
                if range > 0.0 {
                    let y_position = bounds.height - ((last_val - lowest) / range * bounds.height);
                    let label = LabelContent {
