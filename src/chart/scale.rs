...
impl canvas::Program<Message> for AxisLabelsX<'_> {
    ...
    fn draw(
        ...
        let labels = self.labels_cache.draw(renderer, bounds.size(), |frame| {
            ...
            // Draw x-axis crosshair label whenever the cursor x is within the
            // chart's horizontal range, regardless of which vertical pane it is in.
            if let Some(abs_pos) = cursor.position() {
                let cb = self.chart_bounds;
                if abs_pos.x >= cb.x && abs_pos.x <= cb.x + cb.width {
                    // Convert absolute x to chart-local x (same as position_in would give)
                    let local_cursor = iced::Point::new(abs_pos.x - cb.x, cb.height / 2.0);
                    if let Some(label) = self.generate_crosshair(local_cursor, region, bounds, palette)
                    {
                        labels.push(label);
                    }
                }
