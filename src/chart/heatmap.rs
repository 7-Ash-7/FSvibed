...
impl Chart for HeatmapChart {
    ...
    /// Light invalidation for pan events.
    ///
    /// Clears all draw caches (translation is baked into cached geometry)
    /// but deliberately skips the CenterLatest translation-snap inside
    /// `invalidate`.  That snap overrides the user's drag position every
    /// frame, which is what makes panning feel sticky.
            ...
            let crosshair = chart.cache.crosshair.draw(renderer, bounds_size, |frame| {
                ...
                if let Some(cursor_position) = cursor.position_in(bounds) {
                        ...
                        prices_for_display_lookup.iter().enumerate()
                    {
                        ...
                        // sub-panel of this chart. Compute indicator zone height from splits
                        // so the time-axis strip below is excluded.
                        ...
                        if let Some(local_pos) = cursor.position_in(extended) {
                            if local_pos.y >= bounds.height {
                                ...
                                frame.stroke(
                                    &canvas::Path::line(
                                        Point::new(snap_ratio * bounds_size.width, 0.0),
                                        Point::new(snap_ratio * bounds_size.width, bounds_size.height),
                                    ),
                                    dashed_line,
                                );
                            }
