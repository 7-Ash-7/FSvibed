...
impl KlineChart {
        ...
        match &self.data_source {
            PlotData::TimeBased(timeseries) => {
                ...
                if !self.fetching_trades.0 && is_trade_fetch_enabled() {
                    ...
                    // For Hyperliquid: trigger whenever raw_trades is empty (chart has no
                    // backfill yet). The API ignores time ranges so we pass the visible
                    // window; what matters is that the request fires exactly once.
                    // For all exchanges: use gap-detection regardless of chart kind so
                    // that CVD and Volume Profile on candle charts also backfill.
                    let should_fetch = if is_hyperliquid {
                        self.raw_trades.is_empty()
                    } else {
                        timeseries
                    ...
                    if should_fetch {
                        let range = FetchRange::Trades(visible_earliest, visible_latest);
                        if let Some(action) = request_fetch(&mut self.request_handler, range) {
    ...
    pub fn insert_raw_trades(&mut self, raw_trades: Vec<Trade>, is_batches_done: bool, req_id: Option<uuid::Uuid>) {
        match self.data_source {
            PlotData::TickBased(ref mut tick_aggr) => {
        ...
        if is_batches_done {
            ...
            // Historical trade batch is complete: footprints have been filled
            // in for bars that previously had none. Rebuild all indicators so
            // they reflect the newly populated data (e.g. CVD).
            self.indicators
                .values_mut()
                .filter_map(Option::as_mut)
                .for_each(|indi| indi.rebuild_from_source(&self.data_source));
...
fn draw_volume_profile(
    ...
    // Group by rounded price level based on tick_grouping multiplier
    let group_ticks = cfg.tick_grouping.multiplier();
    let tick_step = chart.tick_size;
    // Aggregate buy/sell per grouped price level
    let mut profile: BTreeMap<Price, (f32, f32)> = BTreeMap::new();
    // Group size in raw units
    let group_units = tick_step.units * group_ticks as i64;
    let accumulate_footprint = |trades: &data::chart::kline::KlineTrades, profile: &mut BTreeMap<Price, (f32, f32)>| {
        for (price, group) in &trades.trades {
            // Floor price to nearest group boundary using raw units
    ...
    match data_source {
        PlotData::TimeBased(ts) => {
            ...
            let slice: &[_] = match cfg.range {
                ...
                VolumeProfileRange::Session => &all,
            };
            for dp in slice {
                accumulate_footprint(&dp.footprint, &mut profile);
            }
        }
        PlotData::TickBased(ta) => {
            let slice: &[_] = match cfg.range {
                ...
                VolumeProfileRange::Session => &ta.datapoints,
            };
            for dp in slice {
                accumulate_footprint(&dp.footprint, &mut profile);
            }
    ...
    let max_width = chart.cell_width * 12.0;
