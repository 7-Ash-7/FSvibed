...
use data::chart::kline::{
    CvdConfig, FootprintStudy, TickGrouping, VolumeProfileConfig, VolumeProfileDisplay,
    VolumeProfileRange, VolumeProfileScaling,
};
use data::chart::{
    KlineChartKind,
    heatmap::{self, CoalesceKind},
    kline::{ClusterBarStyle, ClusterKind},
};
use data::layout::pane::VisualConfig;
use data::panel::ladder::{self, BackfillLookback, LookbackUnit, ViewMode};
...
pub fn kline_cfg_view<'a>(
    ...
    let content = match kind {
        KlineChartKind::Candles => {
            let vp_cfg = cfg.volume_profile;
            let enabled = vp_cfg.is_some();
            let enable_toggle = checkbox(enabled)
                .label("Volume Profile")
                .on_toggle(move |on| {
                    Message::VisualConfigChanged(
                        pane,
                        VisualConfig::Kline(data::chart::kline::Config {
                            volume_profile: if on {
                                Some(VolumeProfileConfig::default())
                            } else {
                                None
                            },
                            ..cfg
            ...
            let options = if let Some(vp) = vp_cfg {
                let range_pick = pick_list(
                    VolumeProfileRange::ALL.to_vec(),
                    Some(vp.range),
                    move |new_range| {
                        Message::VisualConfigChanged(
                            pane,
                ...
                let display_pick = pick_list(
                    VolumeProfileDisplay::ALL.to_vec(),
                    Some(vp.display),
                    move |new_display| {
                        Message::VisualConfigChanged(
                            pane,
                ...
                let scaling_pick = pick_list(
                    VolumeProfileScaling::ALL.to_vec(),
                    Some(vp.scaling),
                    move |new_scaling| {
                        Message::VisualConfigChanged(
                            pane,
                ...
                let tick_pick = pick_list(
                    TickGrouping::ALL.to_vec(),
                    Some(vp.tick_grouping),
                    move |new_tg| {
                        Message::VisualConfigChanged(
                            pane,
                            VisualConfig::Kline(data::chart::kline::Config {
                                volume_profile: Some(VolumeProfileConfig {
                                    tick_grouping: new_tg,
                                    ..vp
                                }),
                                ..cfg
                            }),
                ...
                let width_pick = pick_list(
                    vec![6u8, 12, 20, 30, 50, 75, 100],
                    Some(vp.max_width_cells),
                    move |new_width| {
                        Message::VisualConfigChanged(
                            pane,
                            VisualConfig::Kline(data::chart::kline::Config {
                                volume_profile: Some(VolumeProfileConfig {
                                    max_width_cells: new_width,
                                    ..vp
                                }),
                                ..cfg
                            }),
                ...
                column![
                    column![text("Range").size(13), range_pick].spacing(4),
                    column![text("Display").size(13), display_pick].spacing(4),
                    column![text("Scaling").size(13), scaling_pick].spacing(4),
                    column![text("Tick grouping").size(13), tick_pick].spacing(4),
                    column![text("Profile width").size(13), width_pick].spacing(4),
                ]
                .spacing(10)
            } else {
                column![]
            };
            let crosshair_toggle = checkbox(cfg.show_crosshair)
                .label("Show crosshair")
                .on_toggle(move |value| {
                    Message::SetGlobalCrosshair(value)
                });
            let axis_left_toggle = checkbox(cfg.price_axis_left)
                .label("Price axis on left")
                .on_toggle(move |value| {
                    Message::VisualConfigChanged(
                        pane,
                        VisualConfig::Kline(data::chart::kline::Config {
                            price_axis_left: value,
                            ..cfg
                        }),
                        false,
                    )
            ...
            let cvd_section = {
                let ema_toggle = checkbox(cfg.cvd.ema_mode)
                    .label("EMA ratio mode (buy EMA / sell EMA)")
                    .on_toggle(move |on| {
                        Message::VisualConfigChanged(
                            pane,
                            VisualConfig::Kline(data::chart::kline::Config {
                                cvd: CvdConfig { ema_mode: on, ..cfg.cvd },
                                ..cfg
                            }),
                            false,
                        )
                    });
                let period_input = if cfg.cvd.ema_mode {
                    let period_str = if cfg.cvd.ema_period == 0 {
                        String::new()
                    } else {
                        cfg.cvd.ema_period.to_string()
                    };
                    Some(
                        row![
                            text("EMA period").size(13),
                            text_input("2–200", &period_str)
                                .on_input(move |s| {
                                    let v = if s.is_empty() {
                                        0
                                    } else {
                                        s.parse::<u32>().unwrap_or(cfg.cvd.ema_period).clamp(1, 200)
                                    };
                                    Message::VisualConfigChanged(
                                        pane,
                                        VisualConfig::Kline(data::chart::kline::Config {
                                            cvd: CvdConfig { ema_period: v, ..cfg.cvd },
                                            ..cfg
                                        }),
                                        false,
                                    )
                ...
                let mut cvd_col = column![ema_toggle].spacing(8);
                if let Some(row) = period_input {
                    cvd_col = cvd_col.push(row);
                }
                column![text("CVD").size(14), cvd_col].spacing(6)
            };
            split_column![
                column![crosshair_toggle, axis_left_toggle].spacing(12),
                column![enable_toggle, options].spacing(12),
                cvd_section,
                row![
                    space::horizontal(),
        ...
        KlineChartKind::Footprint {
            ...
            let bar_style_picklist = pick_list(
                ClusterBarStyle::ALL,
                Some(bar_style),
                move |new_style| {
                    Message::PaneEvent(pane, Event::ClusterBarStyleSelected(new_style))
                },
            );
            let scaling = {
            ...
            split_column![
                ...
                column![text("Bar style").size(14), bar_style_picklist].spacing(8),
                column![text("Studies").size(14), study_cfg].spacing(8),
                cvd_section,
                row![
                    space::horizontal(),
...
pub fn ladder_cfg_view<'a>(cfg: ladder::Config, pane: pane_grid::Pane) -> Element<'a, Message> {
    let display_options = {
        ...
        let trade_text = checkbox(cfg.show_trade_text)
            .label("Show Volume Figures")
            .on_toggle(move |value| {
                Message::VisualConfigChanged(
                    pane,
                    VisualConfig::Ladder(ladder::Config {
                        show_trade_text: value,
                        ..cfg
                    }),
                    false,
                )
            });
        let cumulative_orders = checkbox(cfg.cumulative_orders)
            .label("Cumulative Limit Orders")
            .on_toggle(move |value| {
                Message::VisualConfigChanged(
                    pane,
                    VisualConfig::Ladder(ladder::Config {
                        cumulative_orders: value,
                        ..cfg
                    }),
                    false,
                )
            });
        let cumulative_trades = checkbox(cfg.cumulative_trades)
            .label("Cumulative Trade Volume")
            .on_toggle(move |value| {
                Message::VisualConfigChanged(
                    pane,
                    VisualConfig::Ladder(ladder::Config {
                        cumulative_trades: value,
                        ..cfg
                    }),
                    false,
                )
            });
        let view_mode_pick = pick_list(
            ViewMode::ALL,
            Some(cfg.view_mode),
            move |new_mode| {
                Message::VisualConfigChanged(
    ...
    let retention_slider = {
        let retention_secs = cfg.trade_retention.as_secs_f32();
        let retention_minutes = if retention_secs > 3600.0 * 24.0 {
            60.0_f32
        } else {
            (retention_secs / 60.0).max(1.0)
        };
        let slider_ui = slider(1.0..=60.0, retention_minutes, move |new_minutes| {
            let mins = new_minutes.round().max(1.0) as u64;
            Message::VisualConfigChanged(
    ...
    let backfill_section = {
        let enabled_checkbox = checkbox(cfg.backfill_enabled)
            .label("Enable Volume Profile Backfill")
            .on_toggle(move |value| {
                Message::VisualConfigChanged(
                    pane,
                    VisualConfig::Ladder(ladder::Config {
        ...
        let amount_str = if cfg.backfill_lookback.amount == 0 {
            String::new()
        } else {
            cfg.backfill_lookback.amount.to_string()
        };
        let amount_input: iced::widget::TextInput<'_, Message, iced::Theme, iced::Renderer> =
            text_input("", &amount_str)
            .width(50)
            .on_input(move |val| {
                let amount = if val.is_empty() {
                    0
                } else {
                    val.parse::<u32>().unwrap_or(cfg.backfill_lookback.amount).max(1)
                };
                Message::VisualConfigChanged(
                    pane,
                    VisualConfig::Ladder(ladder::Config {
                        backfill_lookback: BackfillLookback {
                            amount,
                            ..cfg.backfill_lookback
                        },
                        ..cfg
                    }),
                    false,
        ...
        let unit_picker = pick_list(
            &[LookbackUnit::Hours, LookbackUnit::Days][..],
            Some(cfg.backfill_lookback.unit),
            move |unit| {
                Message::VisualConfigChanged(
                    pane,
                    VisualConfig::Ladder(ladder::Config {
        ...
        let lookback_row: Element<'_, Message> = if cfg.backfill_enabled {
            row![
                text("Lookback:").size(13),
                amount_input,
                unit_picker,
            ]
            .align_y(Alignment::Center)
        ...
        column![
            text("Volume Profile Backfill").size(14),
            enabled_checkbox,
            lookback_row,
        ]
        .spacing(8)
