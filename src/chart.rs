...
pub trait Chart: PlotConstants + canvas::Program<Message> {
    ...
    /// Light invalidation used during panning.
    ///
    /// Clears all draw caches (the translation is baked into cached geometry
    /// so every cache must be cleared) but skips the expensive autoscale
    /// recalculation that `invalidate_all` triggers.  Chart types with
    /// indicators should override this to also clear their indicator caches.
    ///
    /// The default implementation is correct for charts without indicators
    /// (e.g. `HeatmapChart` when `CenterLatest` autoscale must be suppressed
    /// during a drag, handled by its own override).
    fn invalidate_labels(&mut self) {
        self.mut_state().cache.clear_all();
    }
...
pub fn update<T: Chart>(chart: &mut T, message: &Message) {
    match message {
        ...
        Message::Translated(translation) => {
            ...
            // FitToVisible requires a full invalidation: changing which bars
            // are visible alters the y-scale (cell_height / base_price_y),
            // which is recalculated inside invalidate_all().
            //
            // For every other autoscale mode panning does not change the
            // scale — only the viewport offset moves.  We still have to
            // clear all draw caches (frame.translate is baked into cached
            // geometry) but we can skip the expensive visible_price_range
            // scan and the CenterLatest translation-snap that would fight
            // the user's drag gesture.
            if is_fit_to_visible {
                chart.invalidate_all();
            } else {
                chart.invalidate_labels();
            }
            return;
        }
...
pub fn view<'a, T: Chart>(
    ...
    let content = {
        ...
        let axis_left = chart.price_axis_left();

        let main_chart: Element<_> = if axis_left {
            row![
                container(
...
impl ViewState {
    ...
    pub fn draw_last_price_line(
        ...
        if let Some(price) = &self.last_price {
            ...
            // Convert transformed-space y to screen-space y for an unclipped full-width line
            let transformed_y = self.price_to_y(last_price);
            let screen_y = (transformed_y + self.translation.y) * self.scaling
                + bounds.height / 2.0;
