...
pub async fn fetch_recent_trades(ticker_info: TickerInfo) -> Result<Vec<Trade>, AdapterError> {
    ...
    let raw_trades: Vec<HyperliquidTrade> = post_info(&body).await?;
    ...
    // CVD and Volume Profile require chronological ordering
    trades.sort_unstable_by_key(|t| t.time);
    ...
    channel(100, move |mut output| async move {
        ...
        loop {
            match &mut state {
                ...
                State::Connected(websocket) => match tokio::time::timeout(
                ...
                .await
                {
                    Err(_elapsed) => {
                        ...
                        let _ = output
                            .send(Event::Disconnected(
                                exchange,
                                "WebSocket read timeout".to_string(),
                            ))
                            .await;
                    }
