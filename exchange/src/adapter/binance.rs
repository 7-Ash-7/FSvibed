...
pub async fn fetch_trades(
    ...
    // Determine whether from_date is today — if so the zip is incomplete and
    // we must fall back to the live API.  For any past day the zip on disk is
    // the complete authoritative record; no intraday top-up is needed.
    let is_today = from_date == chrono::Utc::now().date_naive();

    if is_today {
        // Today has no complete zip yet — go straight to the live endpoint.
        return fetch_intraday_trades(ticker_info, from_time).await;
    }
...
pub async fn get_hist_trades(
    ...
    // All filesystem checks and directory creation are blocking — run them on
    // the blocking thread pool so they don't stall the async executor.
    let needs_download = {
        let base_zip_path = base_zip_path.clone();
        tokio::task::spawn_blocking(move || {
            std::fs::create_dir_all(base_path)
                .map_err(|e| AdapterError::ParseError(format!("Failed to create directories: {e}")))?;
            Ok::<bool, AdapterError>(!std::fs::metadata(&base_zip_path).is_ok())
        })
        .await
        .map_err(|e| AdapterError::ParseError(format!("spawn_blocking failed: {e}")))??
    };
    ...
    if needs_download {
        ...
        // Download the body asynchronously, then hand off the write to the
        // blocking pool — writing hundreds of MB synchronously would stall the runtime.
        let body = resp.bytes().await.map_err(AdapterError::from)?;
        let base_zip_path_write = base_zip_path.clone();
        tokio::task::spawn_blocking(move || {
            std::fs::write(&base_zip_path_write, &body).map_err(|e| {
                AdapterError::ParseError(format!("Failed to write zip file: {e}, {base_zip_path_write:?}"))
            })
        })
        .await
        .map_err(|e| AdapterError::ParseError(format!("spawn_blocking write failed: {e}")))??;
    } else {
        log::info!("Using cached {}", zip_path);
    }

    // Zip decompression and CSV parsing are entirely blocking (cpu + disk).
    // Offload to the blocking thread pool to keep the async runtime free.
    tokio::task::spawn_blocking(move || {
        ...
        // Historical zip files cover a complete past day — no intraday
        // top-up is needed or wanted.  The caller (fetch_trades) already
        // routes today's requests directly to fetch_intraday_trades, so
        // we will never reach this point for today's date.
        Ok(trades)
    })
    .await
    .map_err(|e| AdapterError::ParseError(format!("spawn_blocking zip failed: {e}")))?
}
